...
/// Lists the stores belonging to a category using
/// `GET /api/categories/{slug}/stores`. Tapping a store opens store details.
...
class _CategoryStoresScreenState extends State<CategoryStoresScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final stores = await _service.getStoresByCategory(widget.category.slug);
      if (!mounted) return;
      setState(() {
        _stores = stores;
  ...
  Widget _buildBody() {
    switch (_state) {
      ...
      case ViewState.empty:
        return EmptyView(
          title: 'No stores yet',
          message: 'There are no stores in this category right now.',
